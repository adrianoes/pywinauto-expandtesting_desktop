...
def test_crete_user_bad_request():
    #start random data number
    randomData = Faker().hexify(text='^^^^^^^^^^^^')
    ...
    # Assertions
    assert success is False, "The 'success' field is not False."
    assert status == 400, f"Expected status 400, but received: {status}"
    assert message == "A valid email address is required", f"Unexpected message: {message}"
...
def test_login_user_bad_request():
    #start random data number
    randomData = Faker().hexify(text='^^^^^^^^^^^^')
    ...
    assert login_status == 400, f"Expected status 400, but received: {login_status}"
    assert login_message == "A valid email address is required", f"Unexpected message: {login_message}"
...
def test_login_user_unauthorized():
    ...
    # Assertions
    assert login_success is False, "The 'success' field is not False."
    assert login_status == 401, f"Expected status 401, but received: {login_status}"
    assert login_message == "Incorrect email address or password", f"Unexpected message: {login_message}"
...
def test_update_user_bad_request():
    ...
    update_profile_curl_command = f'curl -X "PATCH" "https://practice.expandtesting.com/notes/api/users/profile" ' \
                                ...
                                f'-H "x-auth-token: {user_token}" ' \
                                f'-H "Content-Type: application/x-www-form-urlencoded" ' \
                                f'-d "name=a@#&phone={updated_phone}&company={updated_company}" > "{output_file}"'
    ...
    assert patch_status == 400, f"Expected status 400 in PATCH profile, but received: {patch_status}"
    assert patch_message == "User name must be between 4 and 30 characters", f"Unexpected message: {patch_message}"
...
def test_update_user_unauthorized():
    ...
    # Assertions
    assert patch_success is False, "The 'success' field is not False in PATCH profile response."
    assert patch_status == 401, f"Expected status 401 in PATCH profile, but received: {patch_status}"
    assert patch_message == "Access token is not valid or has expired, you will need to login", f"Unexpected message: {patch_message}"
...
def test_update_user_password_bad_request():
    #start random data number
    randomData = Faker().hexify(text='^^^^^^^^^^^^')
    ...
    change_password_curl_command = f'curl -X "POST" "https://practice.expandtesting.com/notes/api/users/change-password" ' \
                                ...
                                f'-H "x-auth-token: {user_token}" ' \
                                f'-H "Content-Type: application/x-www-form-urlencoded" ' \
                                f'-d "currentPassword={current_password}&newPassword=123" > "{output_file}"'
    ...
    assert password_change_status == 400, f"Expected status 400 in password change, but received: {password_change_status}"
    assert password_change_message == "New password must be between 6 and 30 characters", f"Unexpected message: {password_change_message}"
...
def test_update_user_password_unauthorized():
    ...
    # Assertions
    assert password_change_success is False, "The 'success' field is not False in password change response."
    assert password_change_status == 401, f"Expected status 401 in password change, but received: {password_change_status}"
    assert password_change_message == "Access token is not valid or has expired, you will need to login", f"Unexpected message: {password_change_message}"
...
def test_logout_user_unauthorized():
    ...
    assert logout_status == 401, f"Expected status 401 in logout, but received: {logout_status}"
    assert logout_message == "Access token is not valid or has expired, you will need to login", f"Unexpected message: {logout_message}"
...
def test_delete_user_unauthorized():
    ...
    assert delete_status == 401, f"Expected status 401, but received: {delete_status}"
    assert delete_message == "Access token is not valid or has expired, you will need to login", f"Unexpected message: {delete_message}"
